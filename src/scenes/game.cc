...
static const int WORLD_X = 140, PLAYER_MOVE_DELAY = 3, WORLD_Y = 0,
                 WORLD_W = WINDOW_W - WORLD_X, WORLD_H = WINDOW_H;

namespace Scenes {

Game::Game() : IScene("GAME"), World(WORLD_X, WORLD_Y, WORLD_W, WORLD_H) {
  ...
  std::unique_ptr<UI::Text> search_speed_lbl =
      std::make_unique<UI::Text>("Speed", 10, 215, 100, Font::openSansSmall);

  addWidget(std::move(search_speed_lbl));

  // search speed spinner
  std::unique_ptr<UI::Spinner<int>> search_speed_spnr =
      std::make_unique<UI::Spinner<int>>(_search_speed, 5, 240, 65, 25, 0, 10,
                                         1, UI::ST_HORIZONTAL);
...
void Game::renderSelectedEntityType() {
  ...
  case ENT_NONE:
    r = {0, 0, 0, 0};
    std::cerr << "Warning: ENT_NONE is selected as the entity type, but this "
...
void Game::handleInputs() {
  ...
  SDL_Point g = {(mouse_pos.x - World::getRect().x) / BLOCK_W,
                 mouse_pos.y / BLOCK_H};
  if (lmb_down)
    World::spawnEntity(_entity_type, g.x, g.y);
  if (rmb_down)
    World::deleteWall(g.x, g.y);
...
// modify world in any shape or form. helper() returns false when no path is
// found returns false if no path is found
bool Game::getPath(std::function<std::vector<SDL_Point>(
                       World &world, std::vector<SDL_Point> path)>
                       helper) {
...
void Game::renderPath() {
  ...
  for (const SDL_Point pt : _path) {
    SDL_Rect rect = {World::getRect().x + (pt.x * BLOCK_W),
                     World::getRect().y + (pt.y * BLOCK_H), BLOCK_W, BLOCK_H};
